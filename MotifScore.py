...
def variantdiff(genome: str = typer.Option(None, help="fasta file for the genome"),
    ...
    for i in range(len(segments)):
        ...
        np.save(f"{Path(out_file).parent}/single_snps/for_heatmaps/{Path(out_file).stem}_mat.npy", mat)
        np.save(f"{Path(out_file).parent}/single_snps/for_heatmaps/HOXD13_PPM.npy", np.exp(kernels[2*429]))

        tmp = F.conv1d(mat, kernels)

        np.save(f"{Path(out_file).parent}/single_snps/raw_scores/{Path(out_file).stem}_scores_raw.npy", tmp[0,2*429,:])
        ...
        if diff_score == "probNorm":
            if mode == "average":
                ...
                np.save(f"{Path(out_file).parent}/single_snps/normalized_scores/{Path(out_file).stem}_scores_normalized.npy", tmp[0,429,:])
    ...
    names = motif.names
#    print(names)
    print([(i,m) for i, m in enumerate(names) if "HXD13." in m]) 
    ...
    write_output_motif_features(out_file+f"_{nucleotide}_{diff_score}_{mode}", out, new_names, segments.names())
