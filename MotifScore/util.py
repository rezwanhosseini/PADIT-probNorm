...
def readbed(filename, up):
    ...
    return chrs, start, end, gene #peaks
...
#            width = 16
#        else:
#            height = 501 # (length of the sequence)
#            width = 4
...
#                if self.dinucleotide:
#                    batch[j, :, :len(seg)-1] = returnonehot(seg)
#                else:
...
class SegmentDataSeq:
    ...
    def __getitem__(self, i):
        ...
        for i in range(i2-i1):
            if self.id[i].endswith(".fa"):
                print(f"{i} - {self.dir}{self.id[i]}")
                sequence = FastaFile(f"{self.dir}{self.id[i]}")
                for ref in sequence.references:
                    seg = sequence.fetch(ref)
                    print("fwd seq:", seg)
                    print("rc seq:", str(Seq(seg).reverse_complement()))
                self.seq_lengths[i] = len(seg) ## store the length of each seq
                max_h = max(max_h, len(seg))
        print("max kernel length:", max_h)
        
        if self.dinucleotide:
            height = max_h-1 
            width = 16
        else:
            height = max_h # (length of the sequence)
            width = 4
        batch = np.zeros((batchsize, width, height), dtype=np.float32) 
        stats = np.empty((batchsize, self.additional), dtype=np.float32)
        ...
        for i in range(i2-i1):
            if self.id[i].endswith(".fa"):
                ...
                print(i, len(seg))
                #batch[i, :, max_h] = returnonehot(seg)
                if self.dinucleotide:
                    batch[i, :, :len(seg)-1] = returnonehot(seg)
                else:
                    batch[i, :, :len(seg)] = returnonehot(seg)
            else:
                print("not a readable fasta file!")
#        print(batch.shape)
        return torch.from_numpy(batch), stats
...
class SegmentDataBed:
    def __init__(self, bed, batchsize, genome, windowsize, up, dinucleotide=False):
        ...
        self.id = ["_".join([c, str(s), str(e), str(g)]) for c, s, e, g in zip(self.chrs, self.starts, self.ends, self.gene)]
    ...
    def __getitem__(self, i):
        ...
        batch = np.zeros((batchsize, width, height), dtype=np.float32) 
        stats = np.empty((batchsize, self.additional), dtype=np.float32)
        for i, c, new_s, new_e in zip(range(i2-i1), self.chrs[i1:i2],  self.new_starts[i1:i2], self.new_ends[i1:i2]):
        ...
        #if you're looking at peaks this is the code:
        ...
        #           seg = "N"*(self.padding*2)
            ...
            stats[i] = stringstats(seg)
            if self.dinucleotide:
                batch[i, :, :(new_e-new_s)-1] = returnonehot(seg)
            else:
...
if __name__ == "__main__":
    ...
    segments = SegmentDataBed("TestInput/TestHg38.bed", 128, "/data/genomes/human/Homo_sapiens/UCSC/hg38/Sequence/WholeGenomeFasta/genome.fa", kernels.shape[2])
